...
let ageBracketChart=null, genderChart=null, livingChart=null, maritalChart=null;
...
const DONUT_COLORS = ['#1F6F5C','#C98A1F','#C1502E','#55696C'];

function renderPopulationStats(country){
  const items = [
    {code:'A1', label:'Total 60+'},
    {code:'B1', label:'Total 65+'},
    {code:'B3', label:'Total 80+'},
    {code:'B5', label:'Dependency ratio'},
  ];
  const wrap = document.getElementById('population-stat-cards');
  wrap.innerHTML = items.map(it=>{
    const n = demoNum(it.code, country);
    const label = AAAI_DATA.demographics[it.code].label;
    const display = n===null ? 'Pending' : (/ratio/i.test(label) ? n.toFixed(1) : fmtInt(n));
    return `<div class="stat-card"><div class="sc-label">${it.label}</div><div class="sc-value">${display}</div></div>`;
  }).join('');

  const sources = [...new Set(items.map(it=>{
    const row = AAAI_DATA.demographics[it.code];
    const src = (row.sources && row.sources[country]) || '';
    const yr = (row.years && row.years[country]) || '';
    return src ? src + (yr ? ` (${yr})` : '') : '';
  }).filter(Boolean))];
  document.getElementById('population-stat-note').textContent = sources.length ? 'Source: ' + sources.join(' · ') : '';
}

function renderAgeBracketChart(country){
  const ctx = document.getElementById('age-bracket-chart');
  if(ageBracketChart) ageBracketChart.destroy();
  ageBracketChart = new Chart(ctx, {
    type:'bar',
    data:{
      labels:['Aged 60+','Aged 65+','Aged 80+'],
      datasets:[{
        data:[demoNum('A1',country), demoNum('B1',country), demoNum('B3',country)],
        backgroundColor:['#1F6F5C','#C98A1F','#C1502E'],
        borderRadius:2, maxBarThickness:44
      }]
    },
    options:{ indexAxis:'y', plugins:{legend:{display:false}, tooltip:{callbacks:{label:(i)=> i.formattedValue==='null'?'Pending': Number(i.raw).toLocaleString('en-US')}}},
      scales:{ x:{ticks:{callback:v=>(v/1e6).toFixed(1)+'M'}, grid:{color:'#DED2B0'}}, y:{grid:{display:false}} } }
  });
}

function renderGenderChart(country){
  const female = demoNum('A3',country), male = demoNum('A4',country);
  const ctx = document.getElementById('gender-chart');
  if(genderChart) genderChart.destroy();
  genderChart = new Chart(ctx, {
    type:'doughnut',
    data:{ labels:['Female','Male'], datasets:[{ data:[female||0, male||0], backgroundColor:['#C1502E','#2E5FA3'], borderColor:'#FFFDF6', borderWidth:2 }]},
    options:{ plugins:{legend:{display:false}}, cutout:'62%' }
  });
  const total = (female||0)+(male||0);
  const legend = document.getElementById('gender-legend');
  legend.innerHTML = [['Female',female,'#C1502E'],['Male',male,'#2E5FA3']].map(([label,v,color])=>{
    const pct = total ? ((v/total)*100).toFixed(1)+'%' : '—';
    return `<div class="dl-item"><span class="dl-swatch" style="background:${color}"></span><span class="dl-label">${label}</span><span class="dl-value">${pct}</span></div>`;
  }).join('');
}

function renderStatusDonut(canvasId, legendId, existingChart, chartVarSetter, rows){
  if(existingChart) existingChart.destroy();
  const ctx = document.getElementById(canvasId);
  const values = rows.map(r=> r.v===null ? 0 : r.v);
  const chart = new Chart(ctx, {
    type:'doughnut',
    data:{ labels: rows.map(r=>r.label), datasets:[{ data: values, backgroundColor: DONUT_COLORS, borderColor:'#FFFDF6', borderWidth:2 }]},
    options:{ plugins:{legend:{display:false}}, cutout:'62%' }
  });
  chartVarSetter(chart);
  const legend = document.getElementById(legendId);
  legend.innerHTML = rows.map((r,i)=>{
    const display = r.v===null ? (r.raw ? String(r.raw).slice(0,14) : 'Pending') : r.v.toFixed(1)+'%';
    return `<div class="dl-item"><span class="dl-swatch" style="background:${DONUT_COLORS[i]}"></span><span class="dl-label">${r.label}</span><span class="dl-value">${display}</span></div>`;
  }).join('');
}
...
function renderProfiles(){
  ...
  renderPopulationStats(c);
  renderAgeBracketChart(c);
  renderGenderChart(c);
  renderStatusDonut('living-chart','living-legend', livingChart, ch=>livingChart=ch, livingArrangementRows(c));
  renderStatusDonut('marital-chart','marital-legend', maritalChart, ch=>maritalChart=ch, maritalRows(c));
  const livingSrc = [...new Set(livingArrangementRows(c).filter(r=>r.source).map(r=>r.source+(r.year?` (${r.year})`:'')))];
  document.getElementById('panel-living-note').textContent = livingSrc.length ? 'Source: '+livingSrc.join(' · ') : '';
  const maritalSrc = [...new Set(maritalRows(c).filter(r=>r.source).map(r=>r.source+(r.year?` (${r.year})`:'')))];
  document.getElementById('panel-marital-note').textContent = maritalSrc.length ? 'Source: '+maritalSrc.join(' · ') : '';
